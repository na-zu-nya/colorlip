...
import { extractFromBuffer, extractFromFile } from "../adapters/sharp";

const SAMPLES_DIR = path.resolve(import.meta.dirname, "../../samples");

describe("extractFromFile", () => {
  it("JPEG 画像から代表色を抽出できる", async () => {
    const result = await extractFromFile(path.join(SAMPLES_DIR, "photo1.jpg"));
  ...
  it("PNG 画像から代表色を抽出できる", async () => {
    const result = await extractFromFile(path.join(SAMPLES_DIR, "illust1.png"));
    expect(result.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it("複数のサンプル画像で一貫した結果を返す", async () => {
    ...
    for (const file of files) {
      const result = await extractFromFile(path.join(SAMPLES_DIR, file));
      expect(result.length).toBeGreaterThanOrEqual(1);
      for (const color of result) {
        expect(color.hex).toMatch(/^#[0-9A-F]{6}$/);
