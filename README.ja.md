...
`colorlip` は、画像から代表色やコンパクトなパレットを抽出するための TypeScript ファーストなライブラリです。コアは生のピクセルバッファを対象に動作し、ランタイム依存はありません。Node.js では `sharp`、ブラウザでは Canvas API を使うアダプターを利用できます。
...
- 画像統計に基づく適応的なパレット抽出
- CIELAB Delta E による知覚ベースの近似色マージ
- `hex`、`HSL`、`Lab`、`LCH`、`OKLab`、`OKLCH`、CSS カラー文字列、色相カテゴリを含む豊富な出力
- `dominant`、`accent`、`swatches` を返すパレット API
- 生ピクセルデータ向けのプラットフォーム非依存コア
- `sharp` と Canvas 向けの組み込みアダプター
- コアパッケージはランタイム依存なし
